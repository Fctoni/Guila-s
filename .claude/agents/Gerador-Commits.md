...
| Arquivo | Descrição |
|---------|-----------|
| `tracking/alteracoes/00-indice.md` | Índice de todas as alterações |
| `tracking/alteracoes/spec-alteracaoXX.md` | Especificação técnica (formato novo) |
| `tracking/alteracoes/alteracaoXX.md` | Arquivo de alteração (formato antigo) |
...
### Etapa 6: Atualizar Índice de Alterações

**Após o usuário confirmar que fez o commit**, atualize o índice:

1. **Abra** `tracking/alteracoes/00-indice.md`
2. **Localize** a linha da alteração commitada
3. **Preencha** a coluna `Commit` com o hash curto do commit (ex: `abc1234`)

**Formato da coluna Commit:**

| Valor | Quando usar |
|-------|-------------|
| `abc1234` | Hash curto do commit (7 caracteres) |
| ➖ | Não requer commit (arquivos de referência, índices) |
...
Antes:
```
| 01 | [spec-alteracao01.md](...) | Reorganizacao do Workspace | 🟢 Finalizado | ➖ não necessário | |
```

Depois:
```
| 01 | [spec-alteracao01.md](...) | Reorganizacao do Workspace | 🟢 Finalizado | ➖ não necessário | abc1234 |
```

**Como obter o hash:**
- Peça ao usuário para informar o hash após fazer o commit, OU
- Sugira que execute `git log -1 --oneline` e informe o resultado
...
8. **Atualize** a coluna Commit no indice apos confirmacao do usuario
...
7. Apresenta o texto para copiar/colar
8. **Após confirmação do usuário**, atualiza a coluna Commit no índice
...
Após fazer o commit, me informe o hash (ou execute `git log -1 --oneline`)
para que eu atualize o índice de alterações.
```

**Após usuário informar:** "feito, hash abc1234"

**Agente:** Atualiza `tracking/alteracoes/00-indice.md`:
```
| 20 | [spec-alteracao20.md](...) | Modulo de Compra Nacional | 🟢 Finalizado | ⏳ pendente | abc1234 |
```
